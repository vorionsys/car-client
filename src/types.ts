...
import {
  ...
  getTierCode,
  type TrustTierCode,
} from "@vorionsys/shared-constants/tiers";
...
/** Trust tier score ranges — derived from @vorionsys/shared-constants/tiers */
...
> = Object.fromEntries(
  ALL_TIERS.map((tier) => [
    getTierCode(tier),
    { min: TIER_THRESHOLDS[tier].min, max: TIER_THRESHOLDS[tier].max },
  ]),
) as Record<TrustTier, { min: number; max: number }>;

/** Trust tier labels — derived from @vorionsys/shared-constants/tiers */
export const TRUST_TIER_LABELS: Record<TrustTier, string> = Object.fromEntries(
  ALL_TIERS.map((tier) => [getTierCode(tier), TIER_THRESHOLDS[tier].name]),
